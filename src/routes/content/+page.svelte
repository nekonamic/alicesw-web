...
<div class="container mx-auto p-6">
  <h1 class="text-3xl font-bold mb-8">"{kw}"结果：</h1>
        ...
        <div class="card-body">
          ...
          <div class="card-actions justify-end">
            <button
              ...
              onclick={() => handleRead(item.novelId, item.chapterId)}
              >阅读</button
            >
          </div>
        </div>
  ...
  <div class="flex justify-center items-center mt-4">
    <div class="join">
      {#if currentPage === "1"}
        <button class="join-item btn btn-lg" aria-disabled="true">«</button>
      {:else}
        <button
      ...
      {#if Number(currentPage) * Number(PUBLIC_PAGE_SIZE) > data.result.total}
        <button class="join-item btn btn-lg" aria-disabled="true">»</button>
      {:else}
        <button
          class="join-item btn btn-lg"
